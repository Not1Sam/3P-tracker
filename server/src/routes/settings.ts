...
router.put('/', async (req: AuthenticatedRequest, res) => {
  ...
  for (const [key, value] of entries) {
    ...
    await db.insert(userSettings).values({ key, userId, value: String(value), updatedAt: now })
      .onConflictDoUpdate({ target: userSettings.key, set: { value: String(value), updatedAt: now } });
  }
